...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    for (let i = 0; i < 80; i++) {
      ...
      if (window.NexaKS?.getCurrentUser) { client = window.NexaKS; break; }
      await new Promise(r => setTimeout(r, 100));
    }

    if (!client) {
      console.error("[admin] Auth client failed to load after 8s");
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      try {
        ...
        console.warn("[admin] role lookup attempt " + (attempt + 1) + " failed:", e);
      }
    ...
    // Fallback profile so renderAdminInfo() doesn't crash
    if (!currentProfile) {
